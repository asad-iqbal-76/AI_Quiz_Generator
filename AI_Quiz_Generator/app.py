...
def clean_question(question):
    """Clean question text by removing artifacts and ensuring proper format."""
...
def is_question_unique(new_question, existing_questions):
    """Check if a question is sufficiently different from existing ones."""
...
def is_scientist_related_question(question_text):
    # Define a list of keywords/phrases typically related to scientists
    ...
    # Check if any of the phrases exist in the question (case-insensitive)
    return any(re.search(r'\b' + re.escape(keyword) + r'\b', question_text.lower()) for keyword in scientist_related_keywords)
# Function to generate distractors for name-based MCQs
def generate_scientist_distractors(answer, num_distractors=3):
    # Example list of famous scientists (can be expanded dynamically if needed)
    related_scientists = [
    ...
    # Generate a list of distractors by selecting from this list
    distractors = []
    for name in related_scientists:
        if name != answer:  # Ensure the distractor isn't the answer
            distractors.append(name)
        if len(distractors) >= num_distractors:
            break
    
    # If not enough relevant distractors, fall back on general distractors
    if len(distractors) < num_distractors:
...
def generate_distractors(answer, context, num_distractors=3):
    """Generate high-quality distractors for MCQ answers with robust error handling."""
    try:
...
def handle_numeric_answer(answer_text, num_distractors):
    """Generate distractors in the same format as answer_text (numeric or word)."""
    distractors = set()

    # Determine if answer is word or digit form
    try:
    ...
    # Choose perturbations based on magnitude
    if magnitude == 0:
...
def handle_textual_answer(answer_text, context, num_distractors):
    """Handle textual answers with multiple strategies."""
    distractors = set()
    try:
        synonyms = set()
        antonyms = set()
            ...
            try:
                ...
                for question_text in question_list:
                    ...
                    if qtype == 'MCQ':
                        ...
                        # Deduplicate and filter out blanks or any unwanted distractors
                        options = list(dict.fromkeys([opt.strip() for opt in options if opt.strip() and not opt.startswith('Option')]))

                        # Pad options if fewer than 4 after deduplication
                        while len(options) < 4:
                            fallback_words = [
                        ...
                        # Final structure to return in response
                        question_entry = {
                            'type': 'MCQ',
                            'question': question_text,
                            'options': options,
